...
class HP3PARCollector(object):
    def __init__(self, hp_user, hp_password, hp_ip, hp_port):
        ...
        self.list_CIM_classes = ['TPD_DynamicStoragePool', 'TPD_NodeSystem', 'TPD_DriveCage', 'TPD_DiskDrive', 'TPD_CagePowerSupply', 'TPD_NodePowerSupply', 'TPD_Battery', 'TPD_Fan', 'TPD_IDEDrive', 'TPD_PhysicalMemory', 'TPD_SASPort', 'TPD_FCPort', 'TPD_EthernetPort', 'TPD_PCICard']
    ...
    def hp_wbem_connect(self):
        try:
            ...
            wbem_connect = pywbem.WBEMConnection(wbem_url, (self.hp_user, self.hp_password), default_namespace = "root/tpd", no_verification=True, timeout=50)
    ...
    def _get_status_resources(self, hp_connect):
        ...
        try:
            for CIM_class in self.list_CIM_classes:
                state_of_concrete_instances = hp_connect.EnumerateInstances(CIM_class, PropertyList=["OperationalStatus","HealthState", "DeviceID", "ElementName", "Tag", "SerialNumber", "SystemLED", "OtherOperationalStatus", "RemainingCapacity", "Voltage"])
                for instance in state_of_concrete_instances:
                    if ['TPD_Fan', 'TPD_Battery', 'TPD_CagePowerSupply', 'TPD_NodePowerSupply'].count(CIM_class) == 1:
                        ...
                        self.health_metrics[key_health_status].add_metric([STORAGE_NAME], instance["HealthState"])
                        ...
                        self.oper_metrics[key_oper_status].add_metric([STORAGE_NAME], instance["OperationalStatus"][0])
                        ...
                        if CIM_class == 'TPD_Battery':
                            ...
                            if instance["RemainingCapacity"] is not None:
                                ...
                                self.battery_metrics[key_battery_capacity].add_metric([STORAGE_NAME], instance["RemainingCapacity"])
                            if instance["Voltage"] is not None:
                                ...
                                self.battery_metrics[key_battery_voltage].add_metric([STORAGE_NAME], instance["Voltage"])
                    ...
                    if ['TPD_NodeSystem', 'TPD_DriveCage', 'TPD_DiskDrive', 'TPD_DynamicStoragePool', 'TPD_SASPort', 'TPD_FCPort', 'TPD_EthernetPort'].count(CIM_class) == 1:
                        ...
                        if ['TPD_NodeSystem'].count(CIM_class) == 1:
                            ...
                            self.led_metrics[key_system_led].add_metric([STORAGE_NAME], instance["SystemLED"])
                            ...
                            self.other_oper_metrics[key_other_oper_status].add_metric([STORAGE_NAME], instance["OtherOperationalStatus"])
            ...
            hp_logger.info("Error occurs in getting status - {0}".format(oops))
            sys.exit("1100")
    ...
    def _get_overprovisioning(self, hp_connect):
        ...
        try:
            ...
            for cpg in CPGs:
                ...
                self.overprv_metrics[key_overprv].add_metric([STORAGE_NAME], value_overprv)
        except Exception as oops:
...
def main():
    ...
    REGISTRY.register(HP3PARCollector(arguments.hp_user, arguments.hp_password, arguments.hp_ip, arguments.hp_port))
